...
# sdate: every other script in this project inserts these explicitly before importing
# `sdate.*` -- the editable `pip install -e .` this repo's RunAI launcher runs is not
# reliably sufficient on its own in this environment (confirmed: this exact import
# crashed with ModuleNotFoundError without this line).
...
import time
# sdate: lazy -- ambient_utils.eval_utils doesn't exist in every ambient_utils checkout,
# and we never use --track_fid, so don't require it at import time (see usage at
# the `if args.track_fid:` block below, which imports this locally instead).
import PIL
...
def parse_args(input_args=None):
    ...
    # -- sdate: real time-resolved CT projection data, replacing the HF-datasets/
    # imagefolder loading above entirely (see ambient_tweedie_sdxl/data.py). --
    parser.add_argument("--sdate_mov_path", type=str, default=None,
                        help="Path to the .mov frame source, e.g. .../212_Wunderkerze2.mov")
    parser.add_argument("--sdate_memmap_path", type=str, default=None,
                        help="Path to the cached .u16 memmap for the frame source")
    parser.add_argument("--sdate_frame_start", type=int, default=None)
    parser.add_argument("--sdate_frame_end", type=int, default=None)
    parser.add_argument("--sdate_crop_h", type=int, default=128)
    parser.add_argument("--sdate_crop_w", type=int, default=512)
    parser.add_argument("--sdate_extra_noise_dose", type=float, default=0.05)
    parser.add_argument("--sdate_noise_seed", type=int, default=999)
    parser.add_argument("--sdate_native_clean", action="store_true", default=False,
                        help="Literal reproduction of the upstream recipe: use the NATIVE "
                             "(full-dose, un-thinned) reference frame as the 'clean' image "
                             "(exactly like their real LAION photos), with no dataset-level "
                             "synthetic noise injection -- their own timestep_nature-based "
                             "synthetic corruption is the only noise the model ever sees. "
                             "Overrides --sdate_extra_noise_dose/--sdate_noise_seed (ignored).")
    parser.add_argument("--sdate_max_samples", type=int, default=None)
    parser.add_argument("--sdate_placeholder_caption", type=str,
                        default="a grayscale x-ray computed tomography projection image",
                        help="Fixed caption used for every training example (we have no real captions, "
                             "but SDXL's UNet still requires text-conditioning inputs).")
    parser.add_argument("--sdate_latent_cache_path", type=str, default=None,
                        help="Where to persist the precomputed VAE-encoded model_input latents "
                             "(a real, non-trivial GPU cost worth caching across resumes).")
    parser.add_argument(
    ...
    # sdate: these three are referenced in the training loop (`args.run_consistency_everywhere`,
    # `args.with_grad`) but have NO `add_argument` anywhere in the upstream repo -- confirmed by
    # grepping the whole cloned repo, including their own shipped
    # configs/train_low_level_laion10k.yaml (which sets run_consistency_everywhere but not
    # with_grad). Running their exact script as released, with consistency_coeff > 0, hits
    # `torch.set_grad_enabled(args.with_grad)` and crashes with AttributeError. `with_grad=False`
    # matches the standard pattern (ours included, in the now-superseded AmbientTweedieLoss) of
    # NOT tracking gradients through the multi-step chain-building loop, only through the final
    # prediction pair (computed by a separate, ungated forward pass just below this block).
    parser.add_argument("--run_consistency_everywhere", action="store_true", default=True,
                        help="Anchor the consistency chain at whatever above-nature sigma_t the same "
                             "batch already drew for ADSM, rather than always the exact boundary "
                             "(matches their shipped configs' run_consistency_everywhere: True).")
    parser.add_argument("--with_grad", action="store_true", default=False,
                        help="Track gradients through the (num_consistency_steps-1) intermediate "
                             "chain-building steps. Default False, matching standard no-grad-bootstrap "
                             "practice -- the upstream repo has no default for this at all (bug).")
    parser.add_argument("--consistency_kick_in", type=int, default=30000,
                        help="Present for config-file compatibility with their shipped configs. "
                             "NOT ACTUALLY USED anywhere in this training loop (verified: unused in "
                             "the upstream script too) -- consistency is active from step 0 whenever "
                             "consistency_coeff > 0. Kept as a real arg only so a config file setting "
                             "it doesn't error; does not gate anything.")
    ...
    # Sanity checks
    if args.sdate_mov_path is None or args.sdate_frame_start is None or args.sdate_frame_end is None:
        raise ValueError("Need --sdate_mov_path, --sdate_frame_start and --sdate_frame_end "
                         "(this fork replaces the original HF-datasets/imagefolder loading entirely).")
...
def main(args):
    ...
    # -- sdate: real time-resolved CT projection data, replacing the original HF-
    # datasets/imagefolder loading entirely (see ambient_tweedie_sdxl/data.py's module
    # docstring for the two load-bearing differences from their `compute_vae_encodings`:
    # no synthetic noise injection, and a fixed placeholder caption since we have none). --
    from sdate.tr_diffusion.ambient_tweedie_sdxl.data import (
        PrecomputedSDXLAmbientDataset, SDXLAmbientNativeCleanDataset, SDXLAmbientRealDataset,
        precompute_model_inputs,
    )

    with accelerator.main_process_first():
        if args.sdate_native_clean:
            raw_dataset = SDXLAmbientNativeCleanDataset(
                args.sdate_mov_path, memmap_path=args.sdate_memmap_path,
                frame_start=args.sdate_frame_start, frame_end=args.sdate_frame_end,
                crop=(args.sdate_crop_h, args.sdate_crop_w), max_samples=args.sdate_max_samples,
            )
            logger.info(f"sdate: NATIVE-CLEAN dataset has {len(raw_dataset)} frames "
                       f"(literal upstream recipe -- no synthetic noise injected by us)",
                       main_process_only=True)
        else:
            raw_dataset = SDXLAmbientRealDataset(
                args.sdate_mov_path, memmap_path=args.sdate_memmap_path,
                frame_start=args.sdate_frame_start, frame_end=args.sdate_frame_end,
                crop=(args.sdate_crop_h, args.sdate_crop_w), extra_noise_dose=args.sdate_extra_noise_dose,
                noise_seed=args.sdate_noise_seed, max_samples=args.sdate_max_samples,
            )
            logger.info(f"sdate: real dataset has {len(raw_dataset)} frames, "
                       f"sigma_tn_norm={raw_dataset.sigma_tn_norm:.5f}", main_process_only=True)

        logger.info("⏳ Computing VAE embeddings...", main_process_only=True)
        model_inputs = precompute_model_inputs(
            raw_dataset, vae, accelerator.device,
            batch_size=max(1, args.train_batch_size), num_workers=args.dataloader_num_workers,
            cache_path=args.sdate_latent_cache_path,
        )

        tokens_one = tokenize_prompt(tokenizer_one, [args.sdate_placeholder_caption])
        tokens_two = tokenize_prompt(tokenizer_two, [args.sdate_placeholder_caption])

        train_dataset = PrecomputedSDXLAmbientDataset(
            model_inputs, original_size=(args.sdate_crop_h, args.sdate_crop_w), crop_top_left=(0, 0),
            input_ids_one=tokens_one, input_ids_two=tokens_two,
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                def compute_time_ids(original_size, crops_coords_top_left):
                    ...
                    target_size = (args.sdate_crop_h, args.sdate_crop_w)
