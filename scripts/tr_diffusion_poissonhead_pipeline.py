#!/usr/bin/env python3
"""End-to-end pipeline for the two-head Gamma-Poisson NB-NLL ("poisson_head")
baseline variant on wunderkerze2: train from scratch, denoise, reconstruct,
score vs the plain-Huber baseline, and render projection + reconstruction
comparison movies.

Mirrors the recipe already used for the angletime/temporal_raw_pairs ablations
in this project: 8 epochs from scratch, k=1, dose=0.05, frame_start=400000,
frame_end=500000 for training (matches the pre-extracted memmap); reconstruction
scored over frame_start=400300/frame_end=450000, det_bin=1, stride=200
(window_skip=2), real flat/dark + destripe(k=31) correction (same calibration
maps already computed for this dataset), baseline listed FIRST in the variants
dict (the "noisy" arm's dose is read from whichever variant is encountered
first -- see run_windows/reconstruct.py -- so baseline, the real dose-0.05
arm, must lead).

Safe to re-run: every stage is skipped if its output already exists.

    python scripts/tr_diffusion_poissonhead_pipeline.py
"""
...
BASELINE_CKPT = f"{CK}/tr_denoise_baseline_k1_dose005.pt"
...
def stage_train() -> None:
    if Path(CKPT).exists():
        ...
        return
    log("=== stage 1/5: train poisson_head baseline (8 epochs, from scratch) ===")
    t0 = time.time()
    cmd = [
...
def stage_infer() -> None:
    if Path(str(DENOISED_MM) + ".meta.npz").exists():
        ...
        return
    log("=== stage 2/5: denoise full eval range with the poisson_head checkpoint ===")
    t0 = time.time()
    R.denoise_sequence(
...
def stage_reconstruct() -> dict:
    ...
    for arm in ("baseline", TAG, "noisy"):
        m = res["metrics"][arm]
        log(f"  {arm:12s} PSNR {m['psnr'].mean():6.2f}  SSIM {m['ssim'].mean():.3f}")
    ...
    # reconstruction movie: GT | baseline | poissonhead | noisy, middle slice row
...
def stage_projection_movie() -> None:
    if PROJ_MOVIE.exists():
        ...
        return
    log("=== stage 4/5: projection-domain movie (GT | baseline | poissonhead | noisy) ===")
    t0 = time.time()
    variants = {"baseline": BASELINE_MM, TAG: DENOISED_MM}
