...
class GuidedDDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        # 4.5 Apply guidance to pred_original_sample (D, H, W)
        if self.guidance_function is not None:  
            pred_original_sample = normalize_fn(
                self.guidance_function(denormalize_fn(pred_original_sample), timestep)
            )
