...
class ProjectionSliceDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        """
        Return all voxels from randomly sampled slices.
        
        Returns:
            Dict with:
                - 'coords': Tensor of shape (slices_per_batch * voxels_per_slice, 3)
                            with normalized coordinates in [0, 1]
                - 'values': Tensor of shape (slices_per_batch * voxels_per_slice, 1)
        ...
        return self._get_slice_data(slice_indices)
