...
from diffusers.schedulers.scheduling_ddim import ConfigMixin, register_to_config, BaseOutput, randn_tensor, KarrasDiffusionSchedulers, SchedulerMixin
...
class GuidedDDIMScheduler(SchedulerMixin, ConfigMixin):
    """
    `GuidedDDIMScheduler` extends the denoising procedure introduced in denoising diffusion probabilistic models (DDPMs) with
    ...
    def _threshold_sample(self, sample: torch.Tensor) -> torch.Tensor:
        ...
        """
        dtype = sample.dtype
        is_channel_free_volume = sample.ndim in (3, 4)
        if is_channel_free_volume:
            original_shape = sample.shape
            batch_size = 1 if sample.ndim == 3 else sample.shape[0]
        else:
        ...
        # Flatten sample for doing quantile calculation along each image/volume
        if is_channel_free_volume:
    ...
    def step(
        ...
        # 4.5 Apply guidance to pred_original_sample. Volume pipelines may pass
        # (D, H, W), batched (B, D, H, W), or channelled patch tensors here.
        if self.guidance_function is not None:
            ...
            guided_sample = self.guidance_function(denormalize_fn(pred_original_sample), timestep)
            if guided_sample.shape != pred_original_sample.shape:
                raise ValueError(
                    "guidance_function must return the same shape it receives: "
                    f"got {tuple(guided_sample.shape)} for input {tuple(pred_original_sample.shape)}"
                )
            pred_original_sample = normalize_fn(guided_sample).to(
